...
class IndirectManPower extends Component {
  /*handleSignout = async () => {
    try {
  ...
  constructor(props) {
    ...
    this.firestoreRef = firebase.firestore().collection('indirectManPower').where("Date", "==", this.props.navigation.state.params.JSON_ListView_Clicked_Item).where("User", "==", currentUser.uid);
    this.state = {
      isLoading: true,
      projectId:this.props.navigation.state.params.JSON_projectId,
      indirectManPowerArray: []
    };
  }
...
componentDidMount() {
  ...
  getCollection = (querySnapshot) => {
    const indirectManPowerArray = [];
    querySnapshot.forEach((res) => {
      const { ManPowerDescription, Unit, Qty, Description, Location, Contractor, Status } = res.data();
      indirectManPowerArray.push({
        key: res.id,
        res,
        ManPowerDescription,
        Unit,
  ...
  addAcitvityIndirectManPower = () => this.props.navigation.navigate('addAcitvityIndirectManPower')
...
<View style={styles.viewAll}>
    ...
    <View style={styles.welcomebar}>
        <Text style={styles.welcomebartitle}> Indirect Manpower <Text style={{fontSize: 10}}>({this.props.navigation.state.params.JSON_ListView_Clicked_Item}) </Text></Text>
        <Text style={styles.gobackbtn} onPress={this.goBack}>Go Back</Text>        
    </View>

<ScrollView style={styles.scrollcontainer}>
          {
            this.state.indirectManPowerArray.map((item, i) => {
              return ( 

 
 <TouchableOpacity style={styles.directmanPowergrid} onPress={() => {item.Status == '0' ? this.props.navigation.navigate('AcitvityDetailsIndirectManPower', { userkey: `${JSON.stringify(item.key)}`, }) : null  } } >  
    <View style={styles.directmanPowerBoxmain}>
      <View style={styles.directmanPowerBox}>
            <View style={styles.Box1}>
                    <Text style={styles.heading1}>Contractor:</Text> 
            ...
            <View style={styles.Box2}>
                    <Text style={styles.desc}>{item.Contractor}</Text> 
 ...
 <View style={{  position: 'absolute', bottom:10, right:10, cursor:'pointer',  backgroundColor:'#0e2477', borderRadius:40, width:60, height:60,  }}> 
    <Button title="+" onPress={() => this.props.navigation.navigate('addAcitvityIndirectManPower', { JSON_projectId: this.state.projectId, })} titleStyle={{ color: '#fff', marginTop:'8%', fontSize:20, }} type='clear' />
 </View>
...
directmanPowergrid: { width:'96%', alignItems: 'center', justifyContent: 'center', marginLeft:'0%', marginTop:'5%', },
